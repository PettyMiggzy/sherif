...
Custodies the additive 0.25% trader + 0.25% holder [reward](rewards.md) legs and pays capped, Merkle-proven claims in real ETH. Wired to the router via `setRewardVault`. LIVE at `0x03d5d26E492B288e62D897E7dde91af3CceB4347`.

```solidity
function claim(uint256 epoch, address coin, uint8 side, uint256 amount, bytes32[] proof)  // side 0=traders, 1=holders
function pot(address coin, uint256 epoch) view returns (uint128 traderPot, uint128 holderPot)
function currentEpoch() view returns (uint256)
function EPOCH() view returns (uint256)
function postRoot(uint256 epoch, bytes32 root, bytes32 algoHash, string uri)  // poster only, after finality
```

Claims open only after the epoch's challenge window elapses (an early `claim` reverts). The indexer serves the exact `amount` + `proof` per leaf, see [`GET /api/rewards/:addr`](api.md#rewards--vesting-locks).

## TokenVestingLock

One shared, **irrevocable** locker holding many bags on a cliff + linear schedule, the creator's dev bag or **anyone's** (team, whale). No cancel, no owner, no claw-back. LIVE at `0x7453856c3E5f6832dc660e48c7Daa6f46f3355DF`.
...
function release(uint256 id) returns (uint256 amount)   // permissionless; funds always go to the beneficiary
function releasable(uint256 id) view returns (uint256)
function locked(uint256 id) view returns (uint256)
function lockedPercent(uint256 id) view returns (uint256)   // bps of total still locked
function idsOfToken(address token) view returns (uint256[])         // every lock on a coin
function idsOfBeneficiary(address who) view returns (uint256[])
function schedules(uint256 id) view returns (address token, address beneficiary, uint128 total, uint128 released, uint64 start, uint64 cliff, uint64 duration)
```

`create` needs a prior EXACT-amount ERC20 `approve` to the lock. `start=0` means now; `cliffDuration=0` is linear; `cliffDuration==duration` is a hard cliff (nothing until the end, then 100%).
...
| `Accrued` | RewardVault | A reward leg funded: coin, epoch, side, amount |
| `RootPosted` / `Claimed` | RewardVault | Epoch root posted / a claim paid out |
| `ScheduleCreated` | TokenVestingLock | A bag locked: id, token, beneficiary, total, start, cliff, duration |
