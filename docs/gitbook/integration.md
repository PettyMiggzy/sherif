...
const rw = await Pad.rewards(myAddress);                   // { claimable:[…proofs], pending:[…], totals }
for (const c of rw.claimable) await (await Pad.claimReward(c)).wait(); // claim each leaf in real ETH

// ── Vesting locks (see vesting.md) — creator's dev bag OR anyone's (team, whale) ──
await (await Pad.lockMyBag(token, 50, 0, 90)).wait();      // lock 50% of YOUR balance, linear, 90 days
const locks = await Pad.allLocksOf(token);                 // every lock on the coin: [{ beneficiary, total, lockedPct, end }]
await (await Pad.releaseVesting(id)).wait();               // release your vested tokens (permissionless)
