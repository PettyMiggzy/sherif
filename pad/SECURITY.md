...
Everything here is public. No RPC keys, no secrets, and those never leave the
server / your `.env`.
...
1. `staticCall` the exact tx (an `eth_call` simulation), which catches contract
...
Buys/sells and launch are **gated** until these are set, and the UI honestly says
