// ─────────────────────────────────────────────────────────────────────────────
// Robin Labs — pad-v4 (no-pool-forever rewrite) ARC TESTER DEVNET config
//
// Sibling of config-v4-local.js (Robinhood Chain) — same shape, points at a LOCAL Hardhat devnet
// reporting Arc mainnet's real chainId (5042) instead, running pad-v4's OWN fresh contracts with
// the curve geometry RECALIBRATED for Arc's native currency (USDC, ~$1/unit) — see
// pad-v4/scripts/deploy-arc-demo.js's header comment for the full derivation. Addresses below come
// straight out of that deploy script's output (pad/js/deploy.arc.local.json).
//
// DO NOT point this file at a real network. This is for internal team testing only.
// ─────────────────────────────────────────────────────────────────────────────

export const CHAIN = {
  id: 5042,
  hexId: "0x13b2",
  name: "Arc (pad-v4 tester devnet)",
...
/// Fetch pad-v4/scripts/deploy-arc-demo.js's manifest. Throws a clear, catchable error if the
...
export async function loadDeployment() {
  ...
  const res = await fetch("./js/deploy.arc.local.json", { cache: "no-store" });
  if (!res.ok) throw new Error("deploy.arc.local.json not found — run scripts/deploy-arc-demo.js first");
