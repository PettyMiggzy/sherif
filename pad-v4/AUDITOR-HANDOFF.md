...
- **Hook / curve / ambush (verified clean).** Both trade taxes are money-side (currency0); the curve holds token fees → `fundTokenPushed` at graduation; the ambush vault forwards token → its immutable deploy-time `stakingRecipient` (runbook: the treasury, 70/30). No platform-token path.

**Invariant added:** *the platform wallet's pad-token balance is zero* — every token-denominated fee stream terminates at staking / burn, never the treasury. **[round-3 F1 — now CONTRACT-enforced for the pad token]:** the 70% that reaches `DualStaking` is a pad-token reward, and DualStaking's `platformClaimFeeBps` would have skimmed a % of each claim to `platformTreasury` (which `deploy.js` conflates with `platformFeeWallet`). Closed structurally: `DualStaking.claim()` (`:392`) exempts the pad token unconditionally — `fee = asset == address(tokenAsset) ? 0 : amount*bps/BPS`. `tokenAsset` is the launched pad token (the constructor rejects `address(0)`, and ETH is a distinct sentinel), so **no owner setting of `platformClaimFeeBps` and no side-listing choice can route a pad token to the platform key**; money-side (ETH/stock) rewards still carry the fee. `deploy.js` also defaults `STAKING_CLAIM_FEE_BPS = 0` as belt-and-suspenders. The earlier governance caveat no longer applies. Tested end-to-end in `R3F1.platform-token-invariant.demo.test.js` (3 cases) + `DualStaking.adversarial.test.js [F1]` (3 cases, incl. fee-500 unbypassability + narrow money-side exemption). Recorded in `ROBIN-V4-CURVE-ECON.md §53`.

**Operator decisions — now DRIVEN + BUILT (round 2 of the fee model):**
- **ETH LP fee → 100% platform.** `buyLpFloorShareBps` default flipped `2000 → 0` (`scripts/deploy-curve.js`). The deep `RobinFloorVault` is now funded by the **sell-tax floor slice + ambush ETH LP fees** only (documented) — the buy-LP carve to the floor is gone. The split MECHANISM stays in the contract (any `buyLpFloorShareBps ≤ BPS` still works) — only the shipped default changed.
- **Token LP fee → 70% staking / 30% creator-burn.** New per-pad **`RobinTokenTreasury`** (`contracts/pads/RobinTokenTreasury.sol`): `distribute()` (permissionless) forwards 70% to the staking pool and retains 30% as a **public burn reserve**; `burn()` is **creator-only** and sends the reserve to `0x…dEaD` (PadToken is an immutable plain ERC20 with no burn selector, so a dead-address transfer is the retirement). No withdraw/rescue/owner path — token can only leave to staking or dead. **Which streams sink here is PATH-SCOPED [R3-F3]:** PadFactory pads — LockVault sell-leg (`launch.js` wires `setStakingRecipient(treasury)`) + floor `tokenSink`; curve pads — floor `tokenSink` + the ambush token leg (immutable ctor recipient), while on the **standard curve runbook** (`setStaking` before `graduate()`) the LockVault sell-leg and the graduation reservoir are hard-wired **100% to the staking pool** (`registerLaunch` from `curve.staking`; `setStaking` rejects the treasury's splitter shape [R3-N2]). The **Arrow** sub-path graduates with `staking` unset, so its lock registers 0 and the sell-leg is wired **post-hoc to the treasury** (70/30, per `ARROW.md`) — the reservoir still reaches the pool via `setStaking` + `flushStaking`. Tests: `RobinTokenTreasury.test.js` (7).
- **Scope call (flag for the operator):** the 70/30 split applies to ongoing **token LP fees**, NOT the graduation **leftover-reserve dump** (unsold curve supply → stakers), which stays 100% staking. Burning 30% of the reserve would be a large one-time supply burn that also cuts holder rewards — left at 100% staking unless the operator wants otherwise.
- **PadToken untouched:** dead-address burn was chosen over adding `ERC20Burnable` to the shared, deliberately-immutable PadToken. If the operator wants `totalSupply` itself to drop (vs. tokens merely being unspendable at `0x…dEaD`), that is a one-line `ERC20Burnable` follow-up on PadToken + `token.burn()` in the treasury.
