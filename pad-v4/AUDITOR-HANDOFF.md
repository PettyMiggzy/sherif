...
- **Token LP fee → 70% staking / 30% creator-burn.** New per-pad **`RobinTokenTreasury`** (`contracts/pads/RobinTokenTreasury.sol`): `distribute()` (permissionless) forwards 70% to the staking pool and retains 30% as a **public burn reserve**; `burn()` is **creator-only** and sends the reserve to `0x…dEaD` (PadToken is an immutable plain ERC20 with no burn selector, so a dead-address transfer is the retirement). No withdraw/rescue/owner path — token can only leave to staking or dead. **Which streams sink here is PATH-SCOPED [R3-F3]:** PadFactory pads — LockVault sell-leg (`launch.js` wires `setStakingRecipient(treasury)`) + floor `tokenSink`; curve pads — floor `tokenSink` + the ambush token leg (immutable ctor recipient), while the curve-path LockVault sell-leg and the graduation reservoir are hard-wired **100% to the staking pool** (`registerLaunch` from `curve.staking`; `setStaking` rejects the treasury's splitter shape [R3-N2]). Tests: `RobinTokenTreasury.test.js` (7).
