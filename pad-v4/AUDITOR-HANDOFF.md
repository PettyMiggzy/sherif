...
- **Token LP fee → 70% staking / 30% creator-burn.** New per-pad **`RobinTokenTreasury`** (`contracts/pads/RobinTokenTreasury.sol`): every token-side LP fee sinks here; `distribute()` (permissionless) forwards 70% to the staking pool and retains 30% as a **public burn reserve**; `burn()` is **creator-only** and sends the reserve to `0x…dEaD` (PadToken is an immutable plain ERC20 with no burn selector, so a dead-address transfer is the retirement). No withdraw/rescue/owner path — token can only leave to staking or dead. Wired in `launch.js` (LockVault sell-leg + floor `tokenSink` → treasury; ambush → treasury at graduation). Tests: `RobinTokenTreasury.test.js` (7).
