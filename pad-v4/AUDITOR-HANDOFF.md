...
**Invariant added:** *the platform wallet's pad-token balance is zero under the shipped deploy* — every token-denominated fee stream terminates at staking / burn, never the treasury. **[round-3 F1 correction] This is config/wiring-enforced, NOT fully contract-enforced:** the 70% that reaches `DualStaking` is a pad-token reward, and DualStaking's `platformClaimFeeBps` would skim a % of each claim to `platformTreasury` (which `deploy.js` conflates with `platformFeeWallet`). Closed by defaulting `STAKING_CLAIM_FEE_BPS = 0` in `deploy.js` (the platform forgoes the token-denominated staking claim fee); an owner setting a nonzero token claim fee re-opens it (governance caveat). Tested end-to-end in `R3F1.platform-token-invariant.demo.test.js` + `DualStaking.adversarial.test.js [F1]`. A structural fix (route the token claim fee to treasury/burn) is flagged for the external auditor. Recorded in `ROBIN-V4-CURVE-ECON.md §5`.
