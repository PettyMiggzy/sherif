...
- **Pool LP fee:** 1% static. **[fee-model] ETH (buy) LP fee → 100% platform** (`buyLpFloorShareBps = 0` default): the platform keeps all ETH-side LP fees. **TOKEN (sell) LP fee → the per-pad `RobinTokenTreasury`, split 70% staking / 30% burn** — `distribute()` forwards 70% to the staking pool (holder rewards) and retains 30% as a public burn reserve the **creator** burns when they choose (to `0x…dEaD`; they pay the gas). The platform never holds a pad token.
