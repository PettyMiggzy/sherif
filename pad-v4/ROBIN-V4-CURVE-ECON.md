...
- **Platform is ETH-only — never holds pad tokens (enforced by config + wiring; NOT fully contract-enforced — see the honest scope below):** every platform inflow is money-side (currency0/ETH). Buy tax, buffer, referral, the graduation waterfall, and 100% of the ETH LP fee are all ETH. Token-side (currency1) LP fees route to the per-pad `RobinTokenTreasury`, which splits them **70% staking / 30% creator-burn** — never the platform treasury key. Sources into it: `LockVault`'s token leg (parks in `stakingOwed`, reverts rather than falling back to platform per [M-11], recipient = treasury); the `RobinFloorVault` position (parks its token-side fees in-vault, forwards via `sweepTokenFees()` to `tokenSink` = treasury); the ambush vault's token leg. The graduation leftover-reserve dump stays 100% staking (`curve.staking` = the pool; `setStaking` reverts `StakingAssetMismatch` on a non-pool).
  - **[F1] Honest scope — the one path that is config/governance-enforced, not structural.** The 70% that reaches `DualStaking` becomes a pad-token reward; DualStaking's `platformClaimFeeBps` skims a % of each claim to `platformTreasury`, which `deploy.js` wires to the same key as `platformFeeWallet`. So a **nonzero staking claim fee routes pad tokens to the platform key.** The shipped deploy sets `STAKING_CLAIM_FEE_BPS = 0` (the platform forgoes the token-denominated staking claim fee), which makes the invariant hold — but an owner calling `setPlatformClaimFee(>0)` on a pad pool re-opens it. This is a governance caveat, not a contract guarantee. (A structural fix — route the token claim fee to the treasury/burn, or scope the fee to money-side reward assets only — is flagged for the external auditor.)
  - **Invariant tests:** `RobinFloorVault.test.js` (treasury key's token balance exactly unchanged across a collection), `RobinTokenTreasury.test.js` (70/30 + creator-burn + no-withdraw), `DualStaking.adversarial.test.js [F1]` and `R3F1.platform-token-invariant.demo.test.js` (end-to-end: token fee → treasury → 70% pool → claim leaves the platform at ZERO under the fee-0 deploy; a nonzero fee re-opens the leak).
