...
// flag mining (the hook address must still carry 0x28CC in its low 14 bits, or the PoolManager rejects it).
describe("vanity CA — pad tokens end in 1ab5", () => {
  ...
  it("token mining does NOT disturb the hook's 0x28CC flag mining", async () => {
    ...
    expect(BigInt(hookAddr) & 0x3fffn).to.equal(0x28ccn); // hook flags survive
