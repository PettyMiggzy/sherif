...
describe("SIM — production-geometry calibration (start ~$3.4k, graduate ~$34k, ~4.2 ETH, 10x)", () => {
  it("launches at V3 geometry and reproduces the headline market caps + raise on the V4 curve", async () => {
    ...
    const cfg = { name: "Calibrate", symbol: "CAL", decimals: 18, supply: SUPPLY, curveSupply: CURVE, reserveSupply: RESERVE, tickSpacing: TS, startTickMag: 0, creator: creator.address, noPoolForever: false };
    ...
    const predictedToken = ethers.getCreate2Address(await dep.getAddress(), bindSalt(cfg, tokenSalt), ethers.keccak256(tokenInit));
