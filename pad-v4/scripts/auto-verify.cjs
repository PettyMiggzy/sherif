...
// AUTO-VERIFIER (V4 curve suite) — makes every launched curve pad's contracts show up as verified source
// on the explorer, with zero per-coin work. It watches CurvePadFactoryV4 for CurvePadLaunched and submits
// each new token / hook / curve to Blockscout (which auto-detects the per-pad constructor args).
//
//   Run once (backfill everything, then exit):   node scripts/auto-verify.cjs --once
//   Run forever (poll for new pads):             node scripts/auto-verify.cjs
//
// Config (env):
//   RPC_URL        JSON-RPC endpoint            (default: Robinhood Chain public RPC)
//   FACTORY        CurvePadFactoryV4 address    (REQUIRED — the live factory that emits CurvePadLaunched)
...
//   BLOCKSCOUT_URL                              (see lib/blockscout.cjs; default is correct for this chain)
//
// Idempotent: state is persisted, and Blockscout skips anything already verified — safe to restart anytime.
// Read-only on-chain (getLogs only) and needs no private key. It never blocks a launch; verification is a
// best-effort background follow-up.
const fs = require("fs");
...
const START_BLOCK = Number(process.env.START_BLOCK || manifest.curveFactoryBlock || manifest.factoryBlock || 0);
...
const LAUNCHED = IFACE.getEvent("CurvePadLaunched").topicHash;
...
function loadState() {
  try {
    ...
    if (!s.done) s.done = {};
    return s;
  } catch { return { lastBlock: Math.max(0, START_BLOCK - 1), done: {} }; }
}
function saveState(s) {
  fs.writeFileSync(STATE_FILE, JSON.stringify(s, null, 0));
}

// Verify a batch of {addr, kind} jobs, recording successes in state so we never re-submit.
...
// Scan [from,to] for CurvePadLaunched (token + hook + curve) and return the verify jobs. Only the factory
// emits it, so the getLogs filter is address-scoped and cheap even over a wide backfill.
async function scan(provider, from, to) {
  const jobs = [];
  ...
  for (const lg of launched) {
    ...
    jobs.push({ addr: a.token, kind: "token" });
    jobs.push({ addr: a.hook, kind: "hook" });
    jobs.push({ addr: a.curve, kind: "curve" });
  }
  return jobs;
}
...
async function catchUp(provider, state) {
  ...
  for (let lo = from; lo <= safeHead; lo += CHUNK) {
    ...
    const jobs = await scan(provider, lo, hi);
    found += jobs.length;
    if (jobs.length) {
...
async function main() {
  if (!FACTORY) { console.error("FACTORY env is required (the CurvePadFactoryV4 address that emits CurvePadLaunched)"); process.exit(2); }
  const once = process.argv.includes("--once");
  const provider = new ethers.JsonRpcProvider(RPC_URL, undefined, { staticNetwork: true });
  const state = loadState();
  console.log(`auto-verify (v4): factory ${FACTORY} · from block ${state.lastBlock + 1} · verifier ${BLOCKSCOUT}`);
