...
| F-1 | High | The launch token was deployed with plain `CREATE` (nonce-predictable). An attacker could pre-create **and initialize** the token's WETH pool at the next predictable address, making `CurvePool`'s own `initialize()` revert and **permanently bricking** every launch that reuses that address. | Token now deploys via `CREATE2` with a per-launch salt that folds in `block.number`/`block.timestamp`/dev/name/symbol (`LaunchTokenDeployer` + `CurvePadFactory`). The address — and thus the pool — is unpredictable; a griefer would have to win the race for one exact block, and a retry lands a fresh address, so the DoS can't be made permanent. **Revised — `launchWithSalt`.** A creator who mines their coin's address gives that unpredictability up on purpose, so the entropy can no longer be the defence for them. Two things replace it. The factory folds `msg.sender` into the mined salt, so only the creator can reach their own address (the fold inside `LaunchTokenDeployer` cannot do this: on the launch path its `msg.sender` is the factory, one constant address for everyone). And `CurvePool` no longer treats a squatted pool as fatal — `seed()` walks the price back to the start tick with a zero-liquidity swap, which crosses no ticks and trades nothing. It cannot be done in the constructor, where the contract has no code at its own address yet and the swap callback would land on an empty address. A squat the attacker FUNDS is repaired too, and that was the first fix's real gap: an out-of-range position reads as `pool.liquidity() == 0` at the hostile tick, so it cannot be screened out in advance, and a one-wei WETH position planted before the token exists bricked the launch just as effectively. The repair now offers a bounded budget (0.5% of curve supply) so the swap pays through planted liquidity, consuming only what the crossing costs. Every squat an attacker can set up is payable in that direction for a structural reason: before the token has code they can supply only WETH, a WETH-only position must sit entirely on the WETH side of the price, and crossing into it means acquiring WETH by paying token. Beyond the budget the launch reverts and the creator mines another salt. |
