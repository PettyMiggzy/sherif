...
contract BondDeep is IUniswapV3MintCallback, ReentrancyGuard {
    ...
    // [H-5 FIX] The Bounty wall used to start ~AT spot (200 ticks). That is what makes a manipulated poke
    // profitable: shove the price up, hold 20s until the 15s TWAP converges, poke, and the bid wall re-places
    // just under the inflated price — then sell into it. Measured on a real graduated shape: +14% of the wall
    // per 20-second operation. Placing the wall DEEP removes the profit without asking whether the price is
    // honest (a question with no on-chain answer): to sell into a deep wall above true value the attacker must
    // first walk the price up past the depth, which costs market impact he has to unwind.
    // Settable HERE only so the crossing point can be MEASURED; these become constants once a depth is chosen.
    int24 public bountyNear = 200;
    int24 public bountyFar = 6800;

    function setBountyBand(int24 near, int24 far) external { bountyNear = near; bountyFar = far; }
    ...
    function _placeBounty(int24 aboveAnchor, int24 belowAnchor, uint256 wethAmt) internal {
        ...
        (int24 lo, int24 hi,) = _band(above ? aboveAnchor : belowAnchor, above, bountyNear, bountyFar);
