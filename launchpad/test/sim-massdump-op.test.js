...
// ============================================================================
// Operator's mass-dump fear, simulated on a REAL Uniswap v3 fork of Robinhood
// Chain: many wallets buy a fresh coin up ~2x+, then EVERYONE dumps in sequence.
//
// Trades hit the RAW Uniswap v3 pool via SwapProbe, bypassing PadRouter — the
// strongest anti-honeypot proof: even with zero launchpad cooperation the
// underlying pool must honor every sell.
//
// Asserts:
//   (a) every sell tx SUCCEEDS (no revert) — not a honeypot; pool always pays.
//   (b) total ETH paid to all sellers <= total ETH buyers put in (solvency).
//   (c) the LAST seller (worst price, pool most drained) still gets paid > 0.
//
// Run: FORK_RPC=<rpc> npx hardhat test test/sim-massdump-op.test.js
// ============================================================================
const ONE = 10n ** 18n;
const FACTORY = "0x1f7d7550b1b028f7571e69a784071f0205fd2efa";
const WETH = "0x0Bd7D308f8E1639FAb988df18A8011f41EAcAD73";

// Production "let it ride" calibration from scripts/deploy.js (~$34k / ~4.2 ETH):
...
suite("Mass-dump (operator sim, op file) — buy up ~2x then everyone dumps (fork)", function () {
  this.timeout(300000);

  it("every sell succeeds, total out<=in (solvent), last seller still paid", async () => {
    const signers = await ethers.getSigners();
    const [dep, platform, dev] = signers;
    const buyers = signers.slice(3, 15); // 12 independent dumper wallets
    ...
    // skip the anti-snipe window so buys trade freely (normal live coin)
    await ethers.provider.send("evm_increaseTime", [400]);
    await ethers.provider.send("evm_mine", []);

    // ===== PHASE 1: everyone buys, pushing price up =====
    let totalIn = 0n;
    for (const b of buyers) {
      const spend = ethers.parseEther("0.35"); // ~4.2 ETH across 12 wallets -> well up the curve
      ...
      // cap the buy at graduation price so we never tip past the ceiling
      ...
      totalIn += (wBefore - wAfter); // ACTUAL weth consumed
    }
    ...
    // ===== INVARIANTS =====
    // (c) last seller (worst price, pool most drained) still paid
    expect(lastOut, "last seller payout must be non-zero (pool can always pay)").to.be.greaterThan(0n);
    // (a) every payout strictly positive => every sell filled
    for (let i = 0; i < outs.length; i++) {
      expect(outs[i], `seller ${i} received 0 weth`).to.be.greaterThan(0n);
    }
    // (b) SOLVENCY: sellers can't pull out more than buyers put in
