...
//   many wallets buy a fresh coin up ~2x+, then EVERYONE dumps their full bag.
// Asserts:
...
//       nobody can extract more than was deposited).
...
// PadRouter entirely — so this is the strongest anti-honeypot proof: even with zero
// launchpad cooperation, the underlying pool honors every sell.
//
// Run: FORK_RPC=<rpc> npx hardhat test test/sim-massdump-bahmed.test.js
// ============================================================================
const ONE = 10n ** 18n;
...
suite("Mass-dump (bahmed) — buy up ~2x then everyone dumps (real v3 fork)", function () {
  this.timeout(300000);
  ...
  it("every sell succeeds, out<=in (solvent), last seller still paid", async () => {
    ...
    const rc = await (await factory.launch({ name: "DUMPB", symbol: "DUMPB", dev: dev.address, tax: NOTAX })).wait();
    ...
    // skip the anti-snipe window so buys trade freely (this sim is a NORMAL live coin)
    ...
    for (const b of buyers) {
      const spend = ethers.parseEther("0.3"); // ~3.6 ETH total across 12 wallets -> well up the curve
    ...
    // (a) every payout strictly positive => every sell filled (no honeypot)
    ...
    console.log("\n      ===== MASS-DUMP (bahmed) RESULT (real Uniswap v3 fork) =====");
