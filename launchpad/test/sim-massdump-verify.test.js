...
// ============================================================================
// Independent verification of the operator's exact fear on a REAL Uniswap v3
// fork of Robinhood Chain: many wallets buy a fresh coin up ~2x, then EVERYONE
// dumps their full bag in sequence, trading DIRECTLY against the raw v3 pool
// (via SwapProbe) — i.e. with zero launchpad cooperation, the hardest possible
// anti-honeypot test.
//
// Asserts:
//   (a) every sell tx SUCCEEDS (no revert) and yields a strictly positive payout.
//   (b) total ETH out to all sellers <= total ETH buyers put in (solvency).
//   (c) the LAST seller (worst price, pool most drained) still gets non-zero ETH.
//
// Run: FORK_RPC=<rpc> npx hardhat test test/sim-massdump-verify.test.js
// ============================================================================
const ONE = 10n ** 18n;
const FACTORY = "0x1f7d7550b1b028f7571e69a784071f0205fd2efa";
const WETH = "0x0Bd7D308f8E1639FAb988df18A8011f41EAcAD73";

// Production "let it ride" calibration from scripts/deploy.js (~$34k / ~4.2 ETH graduation)
...
suite("Mass-dump verify — buy up ~2x then everyone dumps (raw v3 fork)", function () {
  this.timeout(300000);
  ...
  it("every sell succeeds, out<=in (solvent), last seller still paid", async () => {
    ...
    const rc = await (await factory.launch({ name: "DUMP2", symbol: "DUMP2", dev: dev.address, tax: NOTAX })).wait();
    ...
    // skip past the anti-snipe window so buys trade freely (normal live coin)
    ...
    for (const b of buyers) {
      const spend = ethers.parseEther("0.3");
    ...
    console.log("\n      ===== MASS-DUMP VERIFY (real Uniswap v3 fork) =====");
    ...
    // stash numbers for the runner
    global.__massdump = {
      buyers: buyers.length, totalIn: totalIn.toString(), totalOut: totalOut.toString(),
      priceMultX1000: priceMultX1000.toString(), firstPrice: firstPrice.toString(),
      lastPrice: lastPrice.toString(), lastOut: lastOut.toString(),
    };
  });
});
