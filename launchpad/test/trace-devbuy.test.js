...
describe("dev-buy FDV", function () {
  this.timeout(120000);
  it("$1400 dev buy -> resulting mcap + dev supply", async () => {
    ...
    const buyUsd = 1400;
    const buyEth = ethers.parseEther((buyUsd / ETH_USD).toFixed(6)); // $1400 -> ~0.729 ETH
    ...
    const soldPct = devTok / (0.75 * TOTAL) * 100; // % of the 750M curve the dev now holds
    const circMcap = wethPerToken * devTok * ETH_USD; // price × circulating (only the dev holds so far)

    console.log(`\n      ===== $1400 DEV BUY @ launch (new calibration) =====`);
    console.log(`      spent            : $${buyUsd}  (${ethers.formatEther(buyEth)} ETH)`);
    console.log(`      dev gets         : ${(devTok/1e6).toFixed(1)}M tokens  (${(devTok/TOTAL*100).toFixed(2)}% of 1B, ${soldPct.toFixed(1)}% of the curve)`);
    console.log(`      FDV after buy    : $${fdv.toFixed(0)}   (started at $3376)`);
    console.log(`      circ mcap after  : $${circMcap.toFixed(0)}   (price × dev's tokens)`);
    console.log(`      ===================================================`);
    expect(fdv).to.be.greaterThan(3376);
  });
});
