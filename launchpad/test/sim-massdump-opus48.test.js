...
//       nobody can extract more ETH than was deposited).
...
// PadRouter entirely — the strongest possible anti-honeypot proof: even with zero
// launchpad cooperation, the underlying pool honors every sell.
//
// Run: FORK_RPC=<rpc> npx hardhat test test/sim-massdump-opus48.test.js
// ============================================================================
const ONE = 10n ** 18n;
...
suite("Mass-dump sim (opus48) — buy up ~2x then everyone dumps (real v3 fork)", function () {
  this.timeout(300000);
  ...
  it("every sell succeeds, out<=in (solvent), last seller still paid", async () => {
    ...
    // skip past the anti-snipe window so buys trade freely (this sim is a NORMAL, live coin)
    ...
    for (const b of buyers) {
      ...
      // cap each buy at the graduation price so we never tip into/over the ceiling
    ...
    for (let i = 0; i < buyers.length; i++) {
      ...
      outs.push(out);
      totalOut += out;
      const realized = (out * ONE) / bag; // weth received per token dumped (1e18-scaled)
    ...
    // (a) every payout was strictly positive => every sell filled
