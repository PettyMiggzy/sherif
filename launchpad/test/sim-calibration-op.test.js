...
// OPERATOR SIM (independent). Deploys a fresh CurvePadFactory with the NEW graduation calibration
// (START_TICK_MAG=201600, CURVE_WIDTH=23000, MIN_GRAD_WIDTH=22800) against the REAL Uniswap v3 on a
// Robinhood-Chain fork, reads the params back off-chain, computes START/GRAD mcap purely from the
// on-chain curve ticks, and launches with a 0.5 ETH dev buy to confirm the dev gets ~30% of the
// purchasable curve supply.
//   FORK_RPC=<rpc> npx hardhat test test/sim-calibration-op.test.js
const ONE = 10n ** 18n;
...
suite("OP calibration sim (201600/23000/22800): ~$3.4k start / ~$34k grad + 0.5 ETH dev buy", function () {
  this.timeout(240000);
  ...
  it("(a) reads back calibration and lands start ~$3.4k / grad ~$34k mcap on-chain", async () => {
    ...
    // params straight off the deployed factory
