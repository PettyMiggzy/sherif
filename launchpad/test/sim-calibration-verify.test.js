...
// Independent verification sim of (a) the NEW graduation calibration and (b) a 0.5-ETH dev buy against the
// REAL Uniswap v3 on a Robinhood-Chain fork. We deploy a fresh CurvePadFactory with the new geometry
// (START_TICK_MAG=201600, CURVE_WIDTH=23000, MIN_GRAD_WIDTH=22800), read the params back off-chain, and
// compute START mcap (curve bottom / startTick) and GRAD mcap (ceiling / gradTick) purely from the on-chain
// ticks. Then we launch a coin with a 0.5 ETH dev buy in the SAME tx and assert the dev receives ~30%.
//   FORK_RPC=<rpc> npx hardhat test test/sim-calibration-verify.test.js
...
suite("Calibration verify — NEW params (201600/23000/22800): ~$3.4k start / ~$34k grad + 0.5 ETH dev buy", function () {
  this.timeout(240000);
  ...
  it("(a) reads back the calibration and lands start ~$3.4k / grad ~$34k mcap on-chain", async () => {
    ...
    console.log(`      START_TICK_MAG=${START_TICK_MAG}  CURVE_WIDTH=${CURVE_WIDTH}  MIN_GRAD_WIDTH=${MIN_GRAD_WIDTH}`);
  ...
  it("(b) a 0.5 ETH dev buy delivers ~30% of the curve supply to the dev", async () => {
    ...
    const CURVE_SUPPLY = SUPPLY * 0.75;
