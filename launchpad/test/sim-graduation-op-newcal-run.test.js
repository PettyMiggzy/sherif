...
// buy the curve all the way up to the ceiling so it graduates, and assert the calibration lands where the
// operator intends: ~4.2 ETH raised / ~$34k mcap, split 0.5 creator + 0.5 platform + ~3.2 ETH into the Bond,
// and the pool still trades (buy + sell) after graduation.
//   (own copy — does not modify existing tests)
//   FORK_RPC=<rpc> npx hardhat test test/sim-graduation-op-newcal-run.test.js
const ONE = 10n ** 18n;
...
suite("Graduation sim (op-newcal-run) — NEW calibration (201600 / 23000 / 22800) → ~4.2 ETH / ~$34k", function () {
  this.timeout(240000);

  it("buys the curve to the ceiling, graduates, hits the intended raise/mcap/split; pool still trades", async () => {
    ...
    // ── measure gross raise directly: WETH the buyer actually spent into the curve ─
    const buyerWethBefore = await wethW.balanceOf(buyer.address);
    ...
    const wethSpentIntoCurve = buyerWethBefore - (await wethW.balanceOf(buyer.address));
    ...
    console.log(`      weth into curve : ${f(wethSpentIntoCurve).toFixed(4)} ETH  (buyer spent)`);
    ...
    const tokErc = await ethers.getContractAt(
      ["function approve(address,uint256) returns (bool)"], token);
    ...
    console.log(`      post-graduation buy + sell both succeeded (pool trades, sells not blocked)\n`);
  });
});
