...
// ============================================================================
// Independent review of the operator's exact fear, on a REAL Uniswap v3 fork of
// Robinhood Chain: many wallets buy a fresh coin up ~2x, then EVERYONE dumps.
// Asserts:
...
// PadRouter): the strongest anti-honeypot proof — even with zero launchpad help,
// the underlying pool honors every sell.
//
// Run: FORK_RPC=<rpc> npx hardhat test test/sim-massdump-review.test.js
// ============================================================================
const ONE = 10n ** 18n;
const FACTORY = "0x1f7d7550b1b028f7571e69a784071f0205fd2efa";
const WETH = "0x0Bd7D308f8E1639FAb988df18A8011f41EAcAD73";

// Production "let it ride" calibration from scripts/deploy.js (~$34k / ~4.2 ETH grad).
...
suite("Mass-dump review — buy up ~2x then everyone dumps (fork)", function () {
  this.timeout(300000);
  ...
  it("every sell succeeds, out<=in (solvent), last seller still paid", async () => {
    ...
    // skip past the anti-snipe window so buys trade freely (a normal, live coin)
    ...
    for (const b of buyers) {
      const spend = ethers.parseEther("0.35"); // ~3.5 ETH total across 10 wallets
      ...
      // cap the buy at the graduation price so we never tip over the ceiling
    ...
    for (let i = 0; i < buyers.length; i++) {
      ...
      outs.push(out);
      totalOut += out;
      const realized = (out * ONE) / bag; // weth per token, 1e18-scaled
    ...
    expect(lastOut, "last seller payout must be non-zero").to.be.greaterThan(0n);
    // (a) every payout strictly positive => every sell filled
    for (let i = 0; i < outs.length; i++) {
      expect(outs[i], `seller ${i} received 0 weth`).to.be.greaterThan(0n);
    }
    // (b) SOLVENCY: sellers cannot pull out more ETH than buyers put in
    ...
    console.log("\n      ===== MASS-DUMP REVIEW RESULT (real Uniswap v3 fork) =====");
