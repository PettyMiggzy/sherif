...
// INDEPENDENT graduation simulation with the NEW calibration
// (START_TICK_MAG=201600, CURVE_WIDTH=23000, MIN_GRAD_WIDTH=22800) against the REAL Uniswap v3 on a
// Robinhood-Chain fork. Deploy a fresh CurvePadFactory with that geometry, buy the curve up to the ceiling
// so it graduates, and assert: ~4.2 ETH raised / ~$34k mcap (±15%), 0.5 creator + 0.5 platform + ~3.2 ETH
// into the Bond, and the pool STILL trades (buy + sell) after graduation.
//   (own file — does not touch contracts or existing tests)
//   FORK_RPC=<rpc> npx hardhat test test/sim-graduation-check.test.js
const ONE = 10n ** 18n;
...
suite("Graduation sim (check) — NEW calibration 201600/23000/22800 -> ~4.2 ETH / ~$34k", function () {
  this.timeout(240000);

  it("buys to the ceiling, graduates, hits raise/mcap/split, and still trades after", async () => {
    ...
    const rc = await (await factory.launch({ name: "Chk", symbol: "CHK", dev: dev.address, tax: NOTAX })).wait();
    ...
    // ── buy the curve ALL THE WAY to the ceiling → graduation eligible ──────────
    ...
    const bondRaise = gev.args.raisedWeth; // WETH handed to the Bond (net of the two 0.5 rewards)
