...
// Independent re-run of the NEW graduation calibration + 0.5 ETH dev buy against the REAL Uniswap v3 on a
// Robinhood-Chain fork. Deploys a fresh CurvePadFactory with the new geometry, reads params back on-chain,
// computes START/GRAD mcap from the real curve ticks, then launches with a 0.5 ETH dev buy and asserts the
// dev receives ~30% of the purchasable (750M) curve supply.
//   FORK_RPC=<rpc> npx hardhat test test/sim-cal-devbuy-opusrun.test.js
...
suite("Calibration + dev-buy sim (opus re-run): 201600/23000/22800", function () {
  this.timeout(300000);
  ...
  it("(a) reads back calibration and lands start ~$3.4k / grad ~$34k mcap on-chain", async () => {
    ...
    console.log(`\n      START_TICK_MAG=${stm} CURVE_WIDTH=${cw} MIN_GRAD_WIDTH=${mgw}`);
  ...
  it("(b) 0.5 ETH dev buy delivers ~30% of the 750M curve supply", async () => {
    const [dep, platform, dev] = await ethers.getSigners();
    const factory = await deployStack(dep, platform);
    ...
    global.__pctOfCurve = pctOfCurve;
    global.__pctOfTotal = pctOfTotal;
