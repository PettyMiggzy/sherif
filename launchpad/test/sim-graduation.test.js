...
//   FORK_RPC=<rpc> npx hardhat test test/sim-graduation.test.js
const ONE = 10n ** 18n;
...
suite("Graduation sim — NEW calibration (201600 / 23000 / 22800) lands at ~4.2 ETH / ~$34k", function () {
  this.timeout(240000);

  it("buys the curve to the ceiling, graduates, and hits the intended raise/mcap/split; pool still trades", async () => {
    ...
    const rc = await (await factory.launch({ name: "Sim", symbol: "SIM", dev: dev.address, tax: NOTAX, poolFee: 0, auctionDays: 0 }, { value: ethers.parseEther("0.001") })).wait();
