...
forkSuite("Graduation grief — spot shoved above the ceiling must not block graduation", function () {
  ...
  it("attacker pushes spot past the ceiling; graduate() nudges back and still posts the Bond", async () => {
    ...
    const rc = await (await factory.launch({ name: "Grief", symbol: "GRF", dev: dev.address, tax: NOTAX, poolFee: 0, auctionDays: 0 })).wait();
  ...
  it("HARDER: attacker plants REAL liquidity above the ceiling, then shoves — graduate() still powers through", async () => {
    ...
    await (await router.setFactory(await factory.getAddress())).wait();
    const NOTAX = { buyBps: 125, sellBps: 125, walletBps: 10000, floorBps: 0, burnBps: 0, projectWallet: dev.address };
    const rc = await (await factory.launch({ name: "Grief2", symbol: "GR2", dev: dev.address, tax: NOTAX, poolFee: 0, auctionDays: 0 })).wait();
