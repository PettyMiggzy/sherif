...
describe("CurveLaunchFactory integration", () => {
  it("splits supply 10% vault / 90% curve, curve graduates, vault activates on the real pool", async () => {
    const [dep, dev, platform, buyer] = await ethers.getSigners();
    const WETH = await (await ethers.getContractFactory("MockWETH9")).deploy();
    const V3 = await (await ethers.getContractFactory("MockUniswapV3Factory")).deploy();
    const SHERIFF = await (await ethers.getContractFactory("MockERC20")).deploy(1_000_000n * ONE);
    const staking = await (await ethers.getContractFactory("SheriffStaking")).deploy(await SHERIFF.getAddress());
    ...
    const vd = await (await ethers.getContractFactory("AthVaultDeployer")).deploy();
    const factory = await (await ethers.getContractFactory("CurveLaunchFactory")).deploy(
      await WETH.getAddress(), await V3.getAddress(), await staking.getAddress(),
    ...
    const v = await ethers.getContractAt("AthVault", vault);
    await v.activate();
