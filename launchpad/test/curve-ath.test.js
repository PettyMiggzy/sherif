...
describe("CurveLaunchFactory + OtcVault integration", () => {
  const OTC_PRICE = 3_330_000_000n;       // WETH-wei per 1e18 token (~$10k MC)
  const OTC_BURN_RATIO = 100n * ONE;      // 100 tokens unlocked per 1 $SHERIFF burned

  async function launch() {
    const [dep, dev, platform, buyer, burner] = await ethers.getSigners();
    const WETH = await (await ethers.getContractFactory("MockWETH9")).deploy();
    const V3 = await (await ethers.getContractFactory("MockUniswapV3Factory")).deploy();
    const SHERIFF = await (await ethers.getContractFactory("MockERC20")).deploy(1_000_000n * ONE);
    ...
    const vd = await (await ethers.getContractFactory("OtcVaultDeployer")).deploy();
    const factory = await (await ethers.getContractFactory("CurveLaunchFactory")).deploy(
      await WETH.getAddress(), await V3.getAddress(), await SHERIFF.getAddress(),
      platform.address, dep.address, await td.getAddress(), await cd.getAddress(), await vd.getAddress()
    );
    ...
    return { dep, dev, platform, buyer, burner, WETH, V3, SHERIFF, factory, SUPPLY, token, curve, vault };
  }

  it("splits supply 20% OTC vault / 80% curve on fixed oracle-free terms", async () => {
    const s = await launch();
    const TOK = await ethers.getContractAt("CurveToken", s.token);
    expect(await TOK.totalSupply()).to.equal(s.SUPPLY); // fixed 1B
    expect(await TOK.balanceOf(s.vault)).to.equal(s.SUPPLY / 5n); // 20%
    expect(await TOK.balanceOf(s.curve)).to.equal(s.SUPPLY - s.SUPPLY / 5n); // 80%

    const c = await ethers.getContractAt("BondingCurve", s.curve);
    expect(await c.VIRT_ETH()).to.equal(800000000000000000n); // 0.8 ETH
    expect(await c.GRAD_TARGET()).to.equal(4n * ONE); // graduate at 4 ETH
    // start MC = spotPrice * totalSupply == 1 ETH (~$3k)
    const startMc = (await c.spotPriceE18()) * s.SUPPLY / ONE;
    expect(startMc).to.equal(ONE);

    // vault wired to the right terms
    const v = await ethers.getContractAt("OtcVault", s.vault);
    expect(await v.otcPrice()).to.equal(OTC_PRICE);
    expect(await v.burnRatio()).to.equal(OTC_BURN_RATIO);
    expect(await v.sheriff()).to.equal(await s.SHERIFF.getAddress());
    expect(await v.platform()).to.equal(s.platform.address);
  });

  it("graduates the curve, then the OTC vault opens: burn $SHERIFF -> buy at the fixed price -> ETH to platform", async () => {
    const s = await launch();
    const c = await ethers.getContractAt("BondingCurve", s.curve);
    const v = await ethers.getContractAt("OtcVault", s.vault);

    // graduate: past the anti-snipe window, buy through the 4-ETH target
    await network.provider.send("evm_increaseTime", [400]);
    await network.provider.send("evm_mine");
    await c.connect(s.buyer).buy(0, { value: 5n * ONE }); // caps at 4 ETH raised + refunds the rest
    expect(await c.graduated()).to.equal(true);

    // bind the graduated pool (permissionless)
    await v.activate();
    expect(await v.active()).to.equal(true);
    expect(await v.pool()).to.equal(await c.pool());

    // the mock pool reports mean tick 0 (price 1e18/token >> OTC price) -> window open
    expect(await v.isOpen()).to.equal(true);

    // burner has no allowance until they burn $SHERIFF
    expect(await v.allowanceOf(s.burner.address)).to.equal(0n);

    // give the burner $SHERIFF and approve the vault to pull it for the burn
    await s.SHERIFF.transfer(s.burner.address, 1000n * ONE);
    await s.SHERIFF.connect(s.burner).approve(await v.getAddress(), 1000n * ONE);

    // burn 10 SHERIFF -> unlock 10 * 100 = 1000 tokens of allowance
    const burnAmt = 10n * ONE;
    const wantTokens = 1000n * ONE;
    const cost = wantTokens * OTC_PRICE / ONE; // tokenAmount * otcPrice / 1e18

    const platBefore = await ethers.provider.getBalance(s.platform.address);
    const deadBefore = await s.SHERIFF.balanceOf("0x000000000000000000000000000000000000dEaD");
    const TOK = await ethers.getContractAt("CurveToken", s.token);

    // overpay by 1 wei to exercise the refund path
    await v.connect(s.burner).buyOtc(burnAmt, wantTokens, { value: cost + 1n });

    // $SHERIFF was burned to dead
    expect((await s.SHERIFF.balanceOf("0x000000000000000000000000000000000000dEaD")) - deadBefore).to.equal(burnAmt);
    // buyer received the tokens at the fixed OTC price
    expect(await TOK.balanceOf(s.burner.address)).to.equal(wantTokens);
    // ETH cost went to the platform wallet (refund returned the extra wei)
    expect((await ethers.provider.getBalance(s.platform.address)) - platBefore).to.equal(cost);
    // allowance consumed
    expect(await v.allowanceOf(s.burner.address)).to.equal(0n);
    expect(await v.totalSold()).to.equal(wantTokens);
  });

  it("enforces the per-wallet allowance and the price-gate", async () => {
    const s = await launch();
    const c = await ethers.getContractAt("BondingCurve", s.curve);
    const v = await ethers.getContractAt("OtcVault", s.vault);
    const pool = await ethers.getContractAt("MockUniswapV3Pool", await (async () => {
      await network.provider.send("evm_increaseTime", [400]);
      await network.provider.send("evm_mine");
      await c.connect(s.buyer).buy(0, { value: 5n * ONE });
      await v.activate();
      return await c.pool();
    })());

    // buying more than the burn unlocks reverts
    await s.SHERIFF.transfer(s.burner.address, 1000n * ONE);
    await s.SHERIFF.connect(s.burner).approve(await v.getAddress(), 1000n * ONE);
    const burnAmt = 1n * ONE;             // unlocks 100 tokens
    const tooMany = 101n * ONE;
    await expect(v.connect(s.burner).buyOtc(burnAmt, tooMany, { value: tooMany * OTC_PRICE / ONE }))
      .to.be.revertedWithCustomError(v, "OverAllowance");

    // drive the TWAP mean tick below the OTC price -> window closes
    const tokenIsToken0 = BigInt(s.token) < BigInt(await s.WETH.getAddress());
    await pool.setObserveMeanTick(tokenIsToken0 ? -200000 : 200000);
    expect(await v.isOpen()).to.equal(false);
    await expect(v.connect(s.burner).buyOtc(0, 50n * ONE, { value: 50n * ONE * OTC_PRICE / ONE }))
      .to.be.revertedWithCustomError(v, "NotOpen");
  });
});
