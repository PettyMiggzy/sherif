...
describe("Disperse", function () {
  ...
  it("disperse() sends variable amounts (used by topup) and refunds overpayment", async () => {
    const d = await deploy();
    const list = RECIPIENTS.slice(0, 5);
    const amts = [UNIT, UNIT * 2n, UNIT * 3n, UNIT * 4n, UNIT * 5n];
    const total = amts.reduce((a, b) => a + b, 0n);
    const before = await balancesOf(list);
    await (await d.disperse(list, amts, { value: total + ethers.parseEther("0.01") })).wait();
    const after = await balancesOf(list);
    for (let i = 0; i < 5; i++) expect(after[i] - before[i]).to.equal(amts[i]);
    expect(await ethers.provider.getBalance(await d.getAddress())).to.equal(0n); // fully refunded
  });
});
